...
train_loader = torch.utils.data.DataLoader(train_protein, batch_size=12, shuffle=True)
...
for q in epochs:
    ...
    for j in lrs:
        ...
        ##
        PATH = './models/epoch_lr/mlp_proteomics_reset_' + str(count1) + str(count2) + '.pth'
        torch.save(mlp.state_dict(), PATH)

        ##
        criterion = nn.BCELoss()
        optimizer = optim.SGD(mlp.parameters(), lr=j, momentum=0.9) #adjust momentum too sometime! also normalize inputs maybe
        ...
        ##
        mlp = MLP().to(device)
        mlp.load_state_dict(torch.load('./models/basic/mlp.pth')) #PATH
        mlp.eval()
