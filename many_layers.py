...
n_trials = 10
n_epoch = [280, 140, 280, 500]

#n_epoch = [70, 140, 280, 500]
accuracies = np.zeros((len(n_epoch),n_trials))
for q in range(len(n_epoch)):
    for o in range(n_trials):
        ...
        ##
        train_loader = torch.utils.data.DataLoader(train_protein, batch_size=16, shuffle=True)#, pin_memory=True)
        test_loader = torch.utils.data.DataLoader(test_protein, batch_size=1, shuffle=False)#, pin_memory=True)
        ...
        ##
        #PATH = './models/mlm/mlp_reset.pth'
        #torch.save(mlp.state_dict(), PATH)

        ##
        criterion = nn.BCELoss()
        optimizer = optim.SGD(mlp.parameters(),lr=0.01,momentum=0.9) #for sgd: ,lr=0.001,momentum=0.9
        scheduler = optim.lr_scheduler.StepLR(optimizer,step_size = 60,gamma=0.2) #adjust these parameters

        ##
        mlp.train()
        for epoch in range(n_epoch[q]):  # loop over the dataset multiple times
        ...
        PATH = '/Users/hongb3/PycharmProjects/pythonProject1/lightning_logs/version_1373113/checkpoints/epoch=279-step=1120.ckpt'
